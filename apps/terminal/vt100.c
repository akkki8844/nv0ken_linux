...
static VtCell *screen_cell(Vt100 *v, int col, int row) {
    ...
    if (col < 0 || col >= v->cols || row < 0 || row >= v->rows) return NULL;
...
static void scroll_up(Vt100 *v, int top, int bot, int n) {
    if (!v->use_alt) {
        for (int i = 0; i < n; i++) sb_push_line(v, top);
    }
    VtCell *screen = v->use_alt ? v->alt_screen : v->screen;
    int lines = bot - top + 1 - n;
    if (lines > 0)
        memmove(&screen[top * v->cols],
                &screen[(top + n) * v->cols],
...
static void cursor_clamp(Vt100 *v) {
    if (v->cursor_col < 0)        v->cursor_col = 0;
    if (v->cursor_col >= v->cols) v->cursor_col = v->cols - 1;
    if (v->cursor_row < 0)        v->cursor_row = 0;
    if (v->cursor_row >= v->rows) v->cursor_row = v->rows - 1;
}
    ...
    for (int i = 0; i < count; i++) {
        ...
        switch (p) {
            case 0:
                ...
                v->current_attr.attrs = 0;
                break;
            case 1:  v->current_attr.attrs |=  VT_ATTR_BOLD;          break;
            case 2:  v->current_attr.attrs |=  VT_ATTR_DIM;           break;
            case 3:  v->current_attr.attrs |=  VT_ATTR_ITALIC;        break;
            case 4:  v->current_attr.attrs |=  VT_ATTR_UNDERLINE;     break;
            case 5:  v->current_attr.attrs |=  VT_ATTR_BLINK;         break;
            case 7:  v->current_attr.attrs |=  VT_ATTR_REVERSE;       break;
            case 8:  v->current_attr.attrs |=  VT_ATTR_HIDDEN;        break;
            case 9:  v->current_attr.attrs |=  VT_ATTR_STRIKETHROUGH; break;
            case 22: v->current_attr.attrs &= ~(VT_ATTR_BOLD | VT_ATTR_DIM); break;
            case 23: v->current_attr.attrs &= ~VT_ATTR_ITALIC;        break;
            case 24: v->current_attr.attrs &= ~VT_ATTR_UNDERLINE;     break;
            case 25: v->current_attr.attrs &= ~VT_ATTR_BLINK;         break;
            case 27: v->current_attr.attrs &= ~VT_ATTR_REVERSE;       break;
            case 28: v->current_attr.attrs &= ~VT_ATTR_HIDDEN;        break;
            case 29: v->current_attr.attrs &= ~VT_ATTR_STRIKETHROUGH; break;
            case 39: v->current_attr.fg = DEFAULT_FG;                  break;
            case 49: v->current_attr.bg = DEFAULT_BG;                  break;
            default:
                if (p >= 30 && p <= 37) { v->current_attr.fg = p - 30; break; }
                if (p >= 40 && p <= 47) { v->current_attr.bg = p - 40; break; }
                if (p >= 90 && p <= 97) { v->current_attr.fg = p - 90 + 8; break; }
                if (p >= 100 && p <= 107) { v->current_attr.bg = p - 100 + 8; break; }
...
static int param(Vt100 *v, int idx, int def) {
    if (idx >= v->param_count || v->params[idx] == 0) return def;
    return v->params[idx];
}
...
static void dispatch_csi(Vt100 *v, char final) {
    ...
    switch (final) {
        case 'A': cursor_move(v, v->cursor_col, v->cursor_row - p1); break;
        case 'B': cursor_move(v, v->cursor_col, v->cursor_row + p1); break;
        case 'C': cursor_move(v, v->cursor_col + p1, v->cursor_row); break;
        case 'D': cursor_move(v, v->cursor_col - p1, v->cursor_row); break;

        case 'E': cursor_move(v, 0, v->cursor_row + p1); break;
        case 'F': cursor_move(v, 0, v->cursor_row - p1); break;
        case 'G': cursor_move(v, p1 - 1, v->cursor_row);  break;

        case 'H':
        case 'f': cursor_move(v, p2 - 1, p1 - 1); break;
        ...
        case 'J':
            switch (param(v, 0, 0)) {
                case 0: screen_clear_line(v, v->cursor_row, v->cursor_col, v->cols - 1);
                        screen_clear_rows(v, v->cursor_row + 1, v->rows - 1); break;
                case 1: screen_clear_line(v, v->cursor_row, 0, v->cursor_col);
                        screen_clear_rows(v, 0, v->cursor_row - 1); break;
                case 2: screen_clear_rows(v, 0, v->rows - 1); break;
                case 3: screen_clear_rows(v, 0, v->rows - 1);
                        v->sb_count = 0; v->sb_head = 0; break;
            }
            break;

        case 'K':
            switch (param(v, 0, 0)) {
                case 0: screen_clear_line(v, v->cursor_row, v->cursor_col, v->cols - 1); break;
                case 1: screen_clear_line(v, v->cursor_row, 0, v->cursor_col); break;
                case 2: screen_clear_line(v, v->cursor_row, 0, v->cols - 1); break;
            }
            break;
        ...
        case 'P': {
            ...
            int row = v->cursor_row;
            int n   = p1;
            if (n > v->cols - v->cursor_col) n = v->cols - v->cursor_col;
        ...
        case '@': {
            ...
            if (remain > 0)
                memmove(&screen[row * v->cols + v->cursor_col + n],
                        &screen[row * v->cols + v->cursor_col],
                        sizeof(VtCell) * remain);
            screen_clear_line(v, v->cursor_row, v->cursor_col, v->cursor_col + n - 1);
            break;
        }

        case 'S': scroll_up(v,   v->scroll_top, v->scroll_bot, p1); break;
        case 'T': scroll_down(v, v->scroll_top, v->scroll_bot, p1); break;

        case 'X': screen_clear_line(v, v->cursor_row, v->cursor_col,
                                    v->cursor_col + p1 - 1); break;
        ...
        case 'd': cursor_move(v, v->cursor_col, p1 - 1); break;
        ...
        case 'r':
            ...
            if (v->scroll_bot >= v->rows)    v->scroll_bot = v->rows - 1;
        ...
        case 'h':
            for (int i = 0; i < v->param_count; i++) {
                if (v->interm_count > 0 && v->interm[0] == '?') {
                    switch (v->params[i]) {
                        case 1:    break;
                        case 6:  v->mode_origin       = 1; break;
                        case 7:  v->mode_auto_wrap     = 1; break;
                        case 12: break;
                        case 25: v->mode_cursor_visible= 1; break;
                        case 47:
                        case 1047:
                        case 1049:
                            if (!v->use_alt) {
                                ...
                                memset(v->alt_screen, 0,
                                       sizeof(VtCell) * v->cols * v->rows);
                                screen_clear_rows(v, 0, v->rows - 1);
                                v->use_alt = 1;
                                cursor_move(v, 0, 0);
                            }
                            break;
                        case 2004: v->mode_bracketed_paste = 1; break;
                    }
                } else {
                    switch (v->params[i]) {
                        case 4: v->mode_insert = 1; break;
                    }
                }
            }
            break;
        ...
        case 'l':
            for (int i = 0; i < v->param_count; i++) {
                if (v->interm_count > 0 && v->interm[0] == '?') {
                    switch (v->params[i]) {
                        case 6:  v->mode_origin        = 0; break;
                        case 7:  v->mode_auto_wrap      = 0; break;
                        case 25: v->mode_cursor_visible = 0; break;
                        case 47:
                        case 1047:
                        case 1049:
                            if (v->use_alt) {
                            ...
                            break;
                        case 2004: v->mode_bracketed_paste = 0; break;
                    }
                } else {
                    switch (v->params[i]) {
                        case 4: v->mode_insert = 0; break;
                    }
                }
            }
            break;
...
static void dispatch_osc(Vt100 *v) {
    (void)v;
}
...
static void param_push_digit(Vt100 *v, char c) {
    if (v->param_count == 0) v->param_count = 1;
...
Vt100 *vt100_new(int cols, int rows, int scrollback) {
    ...
    if (!v->alt_screen) { free(v->screen); free(v); return NULL; }
...
void vt100_resize(Vt100 *v, int cols, int rows) {
    ...
    if (!new_screen || !new_alt) { free(new_screen); free(new_alt); return; }

    VtCell blank = cell_blank();
    for (int i = 0; i < cols * rows; i++) { new_screen[i] = blank; new_alt[i] = blank; }
    ...
    if (v->cursor_col >= cols) v->cursor_col = cols - 1;
    if (v->cursor_row >= rows) v->cursor_row = rows - 1;
    v->scroll_top = 0;
    v->scroll_bot = rows - 1;
}
...
VtCell *vt100_scrollback_line(Vt100 *v, int index, int *out_cols) {
    if (!v || index < 0 || index >= v->sb_count) return NULL;
    int idx = (v->sb_head + v->sb_count - 1 - index) % v->sb_cap;
    if (out_cols) *out_cols = v->scrollback[idx].cols;
