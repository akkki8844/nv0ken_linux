...
static void gap_move(Buffer *b, size_t pos) {
  ...
  if (pos < b->gap_start) {
    ...
    memmove(b->data + b->gap_end - move, b->data + pos, move);
    b->gap_end -= move;
    b->gap_start -= move;
  } else {
    size_t move = pos - b->gap_start;
    memmove(b->data + b->gap_start, b->data + b->gap_end, move);
    b->gap_start += move;
    b->gap_end += move;
  }
}
...
static int gap_ensure(Buffer *b, size_t needed) {
  ...
  memmove(new_data + b->gap_end + grow, new_data + b->gap_end,
          b->alloc - b->gap_end);
...
static void lines_rebuild(Buffer *b) {
  ...
  if (b->line_cap < LINE_CACHE_CAP) {
    size_t *nl = xrealloc(b->line_offsets, sizeof(size_t) * LINE_CACHE_CAP);
    if (!nl)
      return;
    b->line_offsets = nl;
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    if (c == '\n') {
      if (b->line_count >= b->line_cap) {
        ...
        size_t *nl = xrealloc(b->line_offsets, sizeof(size_t) * new_cap);
        if (!nl)
          break;
        b->line_offsets = nl;
        b->line_cap = new_cap;
...
static void lines_mark_dirty(Buffer *b) { b->lines_dirty = 1; }
...
static void undo_push(Buffer *b, UndoType type, size_t pos, const char *text,
                      size_t len) {
  if (b->in_undo)
    return;
  if (!b->undo_stack) {
  ...
  if (b->undo_top >= b->undo_cap) {
    ...
    UndoRecord *nr = xrealloc(b->undo_stack, sizeof(UndoRecord) * new_cap);
    if (!nr)
      return;
    b->undo_stack = nr;
    b->undo_cap = new_cap;
...
int buffer_save(Buffer *b, const char *path) {
  ...
  if (b->gap_start > 0) {
    if (fwrite(b->data, 1, b->gap_start, f) != b->gap_start)
      ok = 0;
  }
  if (ok) {
    size_t after = len - b->gap_start;
    if (after > 0) {
      if (fwrite(b->data + b->gap_end, 1, after, f) != after)
        ok = 0;
    }
  }
  fclose(f);
...
char buffer_char_at(Buffer *b, size_t pos) {
  if (!b || pos >= buf_length(b))
    return '\0';
  return b->data[phys(b, pos)];
}

size_t buffer_cursor(Buffer *b) { return b ? b->cursor : 0; }

int buffer_modified(Buffer *b) { return b ? b->modified : 0; }

const char *buffer_filename(Buffer *b) { return b ? b->filename : NULL; }
...
void buffer_pos_to_lc(Buffer *b, size_t pos, int *line, int *col) {
  ...
  if (line)
    *line = found;
  if (col)
    *col = (int)(pos - b->line_offsets[found]);
}
...
size_t buffer_lc_to_pos(Buffer *b, int line, int col) {
  ...
  size_t line_end = (line + 1 < b->line_count) ? b->line_offsets[line + 1] - 1
                                               : buf_length(b);
...
size_t buffer_line_end(Buffer *b, int line) {
  ...
  if (line >= b->line_count)
    line = b->line_count - 1;
  size_t next =
      (line + 1 < b->line_count) ? b->line_offsets[line + 1] : buf_length(b);
...
void buffer_cursor_word_back(Buffer *b) {
  ...
  if (pos == 0)
    return;
  pos--;
  while (pos > 0 && buffer_char_at(b, pos) == ' ')
    pos--;
...
int buffer_cut(Buffer *b, char **out_text, size_t *out_len) {
  if (buffer_copy(b, out_text, out_len) < 0)
    return -1;
...
size_t buffer_find(Buffer *b, size_t from, const char *needle,
  ...
  if (!b || !needle || needle_len == 0)
    return BUFFER_NPOS;
  size_t len = buf_length(b);
  if (from + needle_len > len)
    return BUFFER_NPOS;
