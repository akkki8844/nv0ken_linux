...
static const char *C_KEYWORDS[] = {
    ...
    "_Imaginary", "NULL",     "true",     "false",  NULL};

static const char *C_TYPES[] = {
    "uint8_t",  "uint16_t", "uint32_t", "uint64_t", "int8_t",    "int16_t",
    "int32_t",  "int64_t",  "size_t",   "ssize_t",  "ptrdiff_t", "uintptr_t",
    "intptr_t", "FILE",     "DIR",      "pid_t",    "uid_t",     "gid_t",
    "mode_t",   "off_t",    NULL};
...
static const char *JS_KEYWORDS[] = {
    ...
    "with",     "yield",   "async",      "await",  "of",       "from",
    "=>",       NULL};

static const char *SHELL_KEYWORDS[] = {
    "if",   "then",  "else",   "elif",   "fi",       "for",      "while",
    "do",   "done",  "case",   "esac",   "function", "in",       "return",
    "exit", "echo",  "export", "source", "local",    "readonly", "declare",
    "set",  "unset", "shift",  "trap",   "eval",     NULL};

static const char *ASM_KEYWORDS[] = {
    "mov",     "push", "pop",   "call",  "ret",     "jmp",    "je",     "jne",
    "jl",      "jle",  "jg",    "jge",   "jz",      "jnz",    "cmp",    "test",
    "add",     "sub",  "mul",   "div",   "imul",    "idiv",   "and",    "or",
    "xor",     "not",  "shl",   "shr",   "sar",     "lea",    "nop",    "int",
    "syscall", "iret", "hlt",   "sti",   "cli",     "lgdt",   "lidt",   "ltr",
    "xchg",    "inc",  "dec",   "neg",   "section", "global", "extern", "db",
    "dw",      "dd",   "dq",    "resb",  "resw",    "resd",   "resq",   "times",
    "equ",     "bits", "use64", "use32", NULL};
...
SyntaxLang syntax_detect(const char *filename) {
  ...
  if (strcasecmp(ext, "h") == 0)
    return LANG_C;
  if (strcasecmp(ext, "cpp") == 0)
    return LANG_C;
  if (strcasecmp(ext, "cc") == 0)
    return LANG_C;
  if (strcasecmp(ext, "cxx") == 0)
    return LANG_C;
  ...
  if (strcasecmp(ext, "sh") == 0)
    return LANG_SHELL;
  if (strcasecmp(ext, "bash") == 0)
    return LANG_SHELL;
  if (strcasecmp(ext, "asm") == 0)
    return LANG_ASM;
...
static void token_push(SyntaxState *s, int start, int end,
...
static int kw_match(const char *text, int start, int end, const char **table) {
  int len = end - start;
  ...
  while (i < len) {
    ...
    if (isdigit((unsigned char)line[i]) ||
      ...
      if (line[i] == '0' && i + 1 < len &&
        ...
        while (i < len && (isdigit((unsigned char)line[i]) || line[i] == 'h' ||
                           line[i] == 'H'))
          i++;
      }
      token_push(s, start, i, TOK_NUMBER);
...
static void highlight_markdown(SyntaxState *s, const char *line, int len) {
  ...
  if (line[0] == '#') {
    ...
    while (level < len && line[level] == '#')
      level++;
    token_push(s, 0, len,
               level == 1   ? TOK_HEADING1
               : level == 2 ? TOK_HEADING2
                            : TOK_HEADING3);
    return;
  }
...
static void highlight_json(SyntaxState *s, const char *line, int len) {
  ...
  while (i < len) {
    if (line[i] == '"') {
      ...
      int after = i;
      while (after < len && (line[after] == ' ' || line[after] == '\t'))
        after++;
      SyntaxTokenType tt =
          (after < len && line[after] == ':') ? TOK_FUNCTION : TOK_STRING;
...
void syntax_highlight_line(SyntaxState *s, const char *line, int len) {
  ...
  switch (s->lang) {
  case LANG_C:
    highlight_c(s, line, len);
    break;
  case LANG_PYTHON:
    highlight_python(s, line, len);
    break;
  case LANG_JS:
    highlight_js(s, line, len);
    break;
  case LANG_SHELL:
    highlight_shell(s, line, len);
    break;
  case LANG_ASM:
    highlight_asm(s, line, len);
    break;
  case LANG_MARKDOWN:
    highlight_markdown(s, line, len);
    break;
  case LANG_JSON:
    highlight_json(s, line, len);
    break;
  case LANG_MAKEFILE:
    highlight_makefile(s, line, len);
    break;
  default:
    break;
  }
